<?php
require_once ADO_PATH . '/Doacoes.class.php'; 
require_once DAO_PATH . '/DoacoesDAO.class.php'; 

/**
 * API REST de Doacoes
 */
class DoacoesWS extends WSUtil
{    
    /**
     * 
     * @var \DoacoesWS singleton instance
     */
     ...
     * @return \DoacoesWS
     */
    public static function getInstance(): \DoacoesWS {
     ...
     * 
     * @param string $id
     * @return \DoacoesDTO|null
     */
    public static function getById($id): ?\DoacoesDTO
    {
        $dto = new DoacoesDTO();
        $obj = new DoacoesADO();
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DoacaoSave')) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Doacao')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Doacao,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }

        $dto = new DoacoesDTO();
        $obj = new DoacoesADO();
        $obj_count = new DoacoesADO();
        ...
        {   
            ...
            DoacoesADO::addComparison($dto, 'nome', CMP_INCLUDE_INSIDE, $searchBy);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            DoacoesADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DoacaoRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
