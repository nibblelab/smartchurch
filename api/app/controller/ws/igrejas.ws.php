...
class IgrejasWS extends WSUtil
{
    ...
    /**
     * Obtenha os ids das igrejas por presbitério
     * 
     * @param string $presbiterio id do presbitério
     * @return array
     */
    public static function getIdsByPresbiterio($presbiterio): array
    {
        $dto = new IgrejasDTO();
        $obj = new IgrejasADO();
    ...
    /**
     * Obtenha os ids das igrejas por sínodo
     * 
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'IgrejaSave')) {
        ...
        $dto->organizada = IgrejaOrganizacao::ORGANIZADA;
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'IgrejaSave,InstanciaSave')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'IgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Igreja,InstanciaSave,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Igreja,IgrejaFederacao,IgrejaSinodal,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        IgrejasADO::addComparison($dto, 'organizada', CMP_EQUAL, IgrejaOrganizacao::ORGANIZADA);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'IgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
