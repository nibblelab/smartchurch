<?php
require_once ADO_PATH . '/VotosCtrl.class.php'; 
require_once DAO_PATH . '/VotosCtrlDAO.class.php'; 

/**
 * API REST de VotosCtrl
 */
class VotosCtrlWS extends WSUtil
{
    /**
     * 
     * Cria
    ...
    public function create(): array
    {
        ...
        $dto = new VotosCtrlDTO();
        $obj = new VotosCtrlADO();
        
        $dto->add = true;
        $dto->votacao = NblFram::$context->data->votacao;
        $dto->escrutinio = NblFram::$context->data->escrutinio;
        $dto->votante = NblFram::$context->data->votante;
    ...
    public function all(): array
    {
        ...
        {   
            ...
            VotosCtrlADO::addComparison($dto, '', CMP_INCLUDE_INSIDE, $searchBy);
            VotosCtrlADO::addComparison($dto, '', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            VotosCtrlADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                VotosCtrlADO::addGrouping($dto, $g);
            }
        }
        ...
        if($ok)
        {
            ...
            {
                $it = $obj->next();
                if(!is_null($it->votacao))
                {
                    $result[] = array(
                        'votacao' => $it->votacao,
                        'escrutinio' => $it->escrutinio,
                        'votante' => $it->votante
                    );
                }
            }
    ...
    public function remove(): array
    {
        ...
        $dto->votacao = NblFram::$context->data->id;
        $r = $obj->get($dto);
        if(!is_null($r))
        {
            $r->delete = true;
