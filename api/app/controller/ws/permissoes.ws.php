<?php
require_once ADO_PATH . '/Permissoes.class.php'; 
require_once DAO_PATH . '/PermissoesDAO.class.php'; 

/**
 * API REST de Permissoes
 */
class PermissoesWS extends WSUtil
{
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Permissao')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }

        $dto = new PermissoesDTO();
        $obj = new PermissoesADO();
        $obj_count = new PermissoesADO();
        ...
        {   
            ...
            PermissoesADO::addComparison($dto, 'nome', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
            PermissoesADO::addComparison($dto, 'descricao', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            PermissoesADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                PermissoesADO::addGrouping($dto, $g);
            }
        }
