...
class PastoresWS extends WSUtil
{
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PastorIgrejaSave')) {
        ...
        if($obj->sync())
        {
            if(NblFram::$context->data->disponibilidade == DisponibilidadeOficiais::ATIVO) {
                // pastor está ativo. Crie o contexto de pastor para ele
                $errs = [];
                if(!ContextosWS::create($dto->pessoa, 
                        ...
                        [ References::PASTOR, References::IGREJA, References::CONSELHO ],
                        $errs, 
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PastorIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if($obj->sync())
        {
            $pastor = PastoresWS::getById($dto->id);
            if(!is_null($pastor)) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PastorIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if(!is_null($obj->get($dto)))
        {
            ...
            if(!is_null($pessoa))
            {
                ...
                $contexto = ContextosWS::getByUserAndReference($d->pessoa, $d->ref, References::PASTOR);
                if(!empty($contexto)) 
                {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Dados,PastorIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        OficiaisADO::addComparison($dto, 'tipo', CMP_EQUAL, TipoOficiais::PASTOR);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PastorIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
            ...
            while($obj->hasNext())
            {
                ...
                if(!is_null($it->id))
                {
                    ...
                    $contextos[] = array(
                        ...
                        'ref' => $it->ref,
                        'ref_tp' => [ References::PASTOR, References::IGREJA, References::CONSELHO ]
                    );
                }
            }
