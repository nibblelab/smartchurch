<?php
require_once ADO_PATH . '/Transmissoes.class.php'; 
require_once DAO_PATH . '/TransmissoesDAO.class.php'; 

/**
 * API REST de Transmissoes
 */
class TransmissoesWS extends WSUtil
{
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'TransmissaoIgrejaSave')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        
        $dto = new TransmissoesDTO();
        $obj = new TransmissoesADO();
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'TransmissaoIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'TransmissaoIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'TransmissaoIgreja,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }

        $dto = new TransmissoesDTO();
        $obj = new TransmissoesADO();
        $obj_count = new TransmissoesADO();
        ...
        $ref = ($this->testInputString('ref')) ? NblFram::$context->data->stat : '';
        $ref_tp = ($this->testInputString('ref_tp')) ? NblFram::$context->data->stat : '';
        ...
        {   
            ...
            TransmissoesADO::addComparison($dto, 'video', CMP_INCLUDE_INSIDE, $searchBy);
        }

        if(!empty($stat))
        {   
            $has_at_least_one_filter = true;
            TransmissoesADO::addComparison($dto, 'stat', CMP_EQUAL, $stat);
        }

        if(!empty($ref))
        {   
            $has_at_least_one_filter = true;
            TransmissoesADO::addComparison($dto, 'ref', CMP_EQUAL, $ref);
        }

        if(!empty($ref_tp))
        {   
            $has_at_least_one_filter = true;
            TransmissoesADO::addComparison($dto, 'ref_tp', CMP_EQUAL, $ref_tp);
        }
        
        if(!empty($igreja))
        {   
            $has_at_least_one_filter = true;
            TransmissoesADO::addComparison($dto, 'ref', CMP_EQUAL, $igreja);
            TransmissoesADO::addComparison($dto, 'ref_tp', CMP_EQUAL, References::IGREJA);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            TransmissoesADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                TransmissoesADO::addGrouping($dto, $g);
            }
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'TransmissaoIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
