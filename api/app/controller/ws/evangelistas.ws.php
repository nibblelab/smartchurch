...
class EvangelistasWS extends WSUtil
{
     ...
     * 
     * @param string $id id do evangelista
     * @return object|null
     */
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'EvangelistaIgrejaSave')) {
        ...
        $dto->tipo = TipoOficiais::EVANGELISTA;
        ...
        if($obj->sync())
        {
            if(NblFram::$context->data->disponibilidade == DisponibilidadeOficiais::ATIVO) {
                // evangelista está ativo. Crie o contexto de evangelista para ele
                $errs = [];
                if(!ContextosWS::create($dto->pessoa, 
                        ...
                        [ References::EVANGELISTA, References::IGREJA, References::CONSELHO ], 
                        $errs, 
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'EvangelistaIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if($obj->sync())
        {
            $evangelista = EvangelistasWS::getById($dto->id);
            if(!is_null($evangelista)) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'EvangelistaIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if(!is_null($obj->get($dto)))
        {
            ...
            if(!is_null($pessoa))
            {
                ...
                $contexto = ContextosWS::getByUserAndReference($d->pessoa, $d->ref, References::EVANGELISTA);
                if(!empty($contexto)) 
                {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Dados,EvangelistaIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        $disponibilidade = ($this->testInputString('disponibilidade')) ? NblFram::$context->data->disponibilidade : '';
        ...
        OficiaisADO::addComparison($dto, 'tipo', CMP_EQUAL, TipoOficiais::EVANGELISTA);
        ...
        {   
            $has_at_least_one_filter = true;
            OficiaisADO::addComparison($dto, 'disponibilidade', CMP_EQUAL, $disponibilidade);
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'EvangelistaIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
            ...
            while($obj->hasNext())
            {
                ...
                if(!is_null($it->id))
                {
                    ...
                    $contextos[] = array(
                        ...
                        'ref' => $it->ref,
                        'ref_tp' => [ References::EVANGELISTA, References::IGREJA, References::CONSELHO ]
                    );
                }
            }
