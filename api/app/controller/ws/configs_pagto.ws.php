<?php
require_once ADO_PATH . '/ConfigsPagto.class.php'; 
require_once DAO_PATH . '/ConfigsPagtoDAO.class.php'; 

/**
 * API REST de ConfigsPagto
 */
class ConfigsPagtoWS extends WSUtil
{
    /**
     * 
     * Cria
    ...
    public function create(): array
    {
        ...
        $dto = new ConfigsPagtoDTO();
        $obj = new ConfigsPagtoADO();
        
        $dto->add = true;
        $dto->id = NblFram::$context->data->id;
        $dto->nome = NblFram::$context->data->nome;
        $dto->email = NblFram::$context->data->email;
        $dto->token_prod = NblFram::$context->data->token_prod;
        $dto->token_test = NblFram::$context->data->token_test;
    ...
    public function all(): array
    {
        ...
        {   
            ...
            ConfigsPagtoADO::addComparison($dto, '', CMP_INCLUDE_INSIDE, $searchBy);
            ConfigsPagtoADO::addComparison($dto, '', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            ConfigsPagtoADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                ConfigsPagtoADO::addGrouping($dto, $g);
            }
        }
            ...
            while($obj->hasNext())
            {
                ...
                if(!is_null($it->id))
                {
                    $result[] = array(
                        ...
                        'nome' => $it->nome,
                        'email' => $it->email,
                        'token_prod' => $it->token_prod,
                        'token_test' => $it->token_test,
                        'ref' => $it->ref,
                        'ref_tp' => $it->ref_tp
                    );
                }
