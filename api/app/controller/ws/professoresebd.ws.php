 ...
 */
class ProfessoresEBDWS extends WSUtil
{
    /**
     * 
     * @var \ProfessoresEBDWS singleton instance
     */
     ...
     * 
     * @return \ProfessoresEBDWS
     */
    public static function getInstance(): \ProfessoresEBDWS {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfessorEBDSave')) {
        ...
        $dto->tipo = TipoOficiais::PROFESSOR;
        ...
        if($obj->sync())
        {
            /* crie o contexto de ebd. Verifique antes */
            $errs = [];
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfessorEBDBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if($obj->sync())
        {
            $superintendente = ProfessoresEBDWS::getById($dto->id);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfessorEBD')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfessorEBDRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
