<?php
require_once ADO_PATH . '/Sinodos.class.php'; 
require_once DAO_PATH . '/SinodosDAO.class.php'; 

/**
 * API REST de Sinodos
 */
class SinodosWS extends WSUtil
{
    /**
     * 
     * Cria
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'SinodoSave')) {
        ...
        $dto->nacional = NblFram::$context->data->nacional;
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'SinodoBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
     ...
     * 
     * @param string $id
     * @return \SinodosDTO|null
     */
    public static function getById($id): ?\SinodosDTO
    {
        $dto = new SinodosDTO();
        $obj = new SinodosADO();
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Sinodo,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    public function all(): array
    {
        ...
        {   
            ...
            SinodosADO::addComparison($dto, 'nome', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
            SinodosADO::addComparison($dto, 'sigla', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
        }

        if(!empty($stat))
        {   
            $has_at_least_one_filter = true;
            SinodosADO::addComparison($dto, 'stat', CMP_EQUAL, $stat);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            SinodosADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'SinodoRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
