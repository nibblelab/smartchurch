...
class DiaconosWS extends WSUtil
{
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DiaconoIgrejaSave')) {
        ...
        if($obj->sync())
        {
            // crie o contexto de administração do diácono
            $errs = [];
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DiaconoIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if($obj->sync())
        {
            $diacono = DiaconosWS::getById($dto->id);
            if(!is_null($diacono)) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DiaconoIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if(!is_null($obj->get($dto)))
        {
            ...
            if(!is_null($pessoa))
            {
                ...
                $contexto = ContextosWS::getByUserAndReference($d->pessoa, $d->ref, References::JUNTA);
                if(!empty($contexto)) 
                {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Dados,DiaconoIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        OficiaisADO::addComparison($dto, 'tipo', CMP_EQUAL, TipoOficiais::DIACONO);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'DiaconoIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if(!is_null($r))
        {
            ...
            {
                // remova o contexto 
                ContextosWS::removeByUserAndInstancia($r->pessoa, InstanciasWS::getIdbyRef($r->ref, References::IGREJA));
