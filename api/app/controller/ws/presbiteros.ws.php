...
class PresbiterosWS extends WSUtil
{
     ...
     * 
     * @param string $id id do presbítero
     * @return object|null
     */
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PresbiteroIgrejaSave')) {
        ...
        $dto->tipo = TipoOficiais::PRESBITERO;
        ...
        if($obj->sync())
        {
            // crie o contexto de administração do presbítero
            $errs = [];
            if(!ContextosWS::create($dto->pessoa, 
                    ...
                    [ References::IGREJA, References::CONSELHO ], 
                    $errs, 
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PresbiteroIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if($obj->sync())
        {
            $presbitero = PresbiterosWS::getById($dto->id);
            if(!is_null($presbitero)) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PresbiteroIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        if(!is_null($obj->get($dto)))
        {
            ...
            if(!is_null($pessoa))
            {
                ...
                $contexto = ContextosWS::getByUserAndReference($d->pessoa, $d->ref, References::CONSELHO);
                if(!empty($contexto)) 
                {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Dados,PresbiteroIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
        ...
        OficiaisADO::addComparison($dto, 'tipo', CMP_EQUAL, TipoOficiais::PRESBITERO);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'PresbiteroIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
            ...
            while($obj->hasNext())
            {
                ...
                if(!is_null($it->id))
                {
                    ...
                    $contextos[] = array(
                        ...
                        'ref' => $it->ref,
                        'ref_tp' => [ References::IGREJA, References::CONSELHO ]
                    );
                }
            }
