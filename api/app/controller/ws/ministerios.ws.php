...
class MinisteriosWS extends WSUtil
{
    ...
    /**
     * Obtenha os ids dos ministérios da igreja
     * 
    ...
    public static function getIdsByIgreja($igreja): array
    {
        ...
        MinisteriosADO::addComparison($dto, 'igreja', CMP_EQUAL, $igreja);
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'MinisterioIgrejaSave')) {
        ...
        if($obj->sync())
        {
            // crie a instância para o ministério, ligado à igreja
            $errs = [];
            $id_instancia = '';
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'MinisterioIgrejaBlock')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'MinisterioIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Dados,MinisterioIgreja')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }

        $dto = new MinisteriosDTO();
        $obj = new MinisteriosADO();
        $obj_count = new MinisteriosADO();
        ...
        {   
            ...
            MinisteriosADO::addComparison($dto, 'nome', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
            MinisteriosADO::addComparison($dto, 'email', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
            MinisteriosADO::addComparison($dto, 'telefone', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
            MinisteriosADO::addComparison($dto, 'ramal', CMP_INCLUDE_INSIDE, $searchBy, OP_OR);
        }

        if(!empty($stat))
        {   
            $has_at_least_one_filter = true;
            MinisteriosADO::addComparison($dto, 'stat', CMP_EQUAL, $stat);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            MinisteriosADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                MinisteriosADO::addGrouping($dto, $g);
            }
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'MinisterioIgrejaRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
