<?php
require_once ADO_PATH . '/Profissoes.class.php'; 
require_once DAO_PATH . '/ProfissoesDAO.class.php'; 

/**
 * API REST de Profissoes
 */
class ProfissoesWS extends WSUtil
{
     ...
     * 
     * @param string $id
     * @return \ProfissoesDTO|null
     */
    public static function getById($id): ?\ProfissoesDTO
    {
        $dto = new ProfissoesDTO();
        $obj = new ProfissoesADO();
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfissaoSave')) {
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Profissao')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'Profissao,Dados')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }

        $dto = new ProfissoesDTO();
        $obj = new ProfissoesADO();
        $obj_count = new ProfissoesADO();
        ...
        {   
            ...
            ProfissoesADO::addComparison($dto, 'nome', CMP_INCLUDE_INSIDE, $searchBy);
        }
        ...
        if(!empty($orderBy))
        {
            ...
            ProfissoesADO::addOrdering($dto, $order_by_v[0], ($order_by_v[1] == 'asc') ? ORDER_ASC : ORDER_DESC);
        }
        ...
        if(!empty($groupBy))
        {
            ...
            foreach($groups as $g) {
                ProfissoesADO::addGrouping($dto, $g);
            }
        }
    ...
    {
        if(!doIHavePermission(NblFram::$context->token, 'ProfissaoRemove')) {
            return array('status' => 'no', 'success' => false, 'msg' => 'Você não tem permissão para executar essa operação');
        }
