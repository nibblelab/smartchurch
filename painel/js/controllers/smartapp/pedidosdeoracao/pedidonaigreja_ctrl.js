angular.module('SmartChurchPanel').controller('PedidosDeOracaoNaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, 
                    $dialogs, $timeout,
                    Data, PedidoDeOracao) { 
    ...
    $scope.objForm = 'Pedido de Oração';
    $scope.objList = 'Pedidos de Oração';
    ...
    $scope.navStates = {
        add: 'SmartChurchPanel.smartapp.pedidosdeoracaonaigreja.adicionar',
        edit: 'SmartChurchPanel.smartapp.pedidosdeoracaonaigreja.editar',
        search: 'SmartChurchPanel.smartapp.pedidosdeoracaonaigreja.buscar'
    };
    
    $scope.localPerms = {
        add: 'PedidosOracaoIgrejaSave',
        edit: 'PedidosOracaoIgrejaSave',
        remove: 'PedidosOracaoIgrejaRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_pedidosdeoracaonaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        PedidoDeOracao.getAllForIgreja($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', 
                                    $scope.stat, $scope.user.id, $scope.igreja).then(function(r) {
            if (r.datas.length > 0) {
    ...
    $scope.dataFrm = {
        ...
        data: {
            ...
            pedido: { value: '', notEmpty: true, valid: true, StringfyFrom: '' }
        },
        toSend: {
            id: '',
            igreja: $scope.igreja,
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            PedidoDeOracao.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) {
                $state.go($scope.navStates.back); 
            });
        });
    }
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = PedidoDeOracao.createForIgreja($scope.dataFrm.toSend);
            }
            else
            {
                promise = PedidoDeOracao.edit($scope.dataFrm.toSend.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            PedidoDeOracao.remove(d.id).then(function() {
                $timeout(function() {
