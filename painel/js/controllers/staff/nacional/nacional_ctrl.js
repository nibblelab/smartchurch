angular.module('SmartChurchPanel').controller('NacionalCtrl', function ($scope, $state, $stateParams, $localstorage, 
                    $notifications, $rootScope, $dialogs, $timeout, $cache,
                    Data, Supremo, Nacional) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.nacionais.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Nacional';
    $scope.objList = 'Nacionais';
    ...
    $scope.localPerms = {
        add: 'NacionalSave',
        edit: 'NacionalSave',
        changeStat: 'NacionalBlock',
        remove: 'NacionalRemove'
    };
    ...
    $scope.listSupremo = [];
    Supremo.getAll().then(function(r) {
        if(r.total > 0) {
            $scope.listSupremo = r.datas;
        }
    ...
    /* search */
    $scope.storage_cache_name = 'search_nacionais';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Nacional.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat).then(function(r) {
            if (r.datas.length > 0) {
    ...
    $scope.dataFrm = {
        ...
        prepare: function() {
            ...
            $scope.dataFrm.toSend.supremo_conciclio = $scope.listSupremo[0].id;
        }
    };
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Nacional.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Nacional.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Nacional.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Nacional.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Nacional.remove(d.id).then(function() {
                $timeout(function() {
