angular.module('SmartChurchPanel').controller('PontoDePregacaoCtrl', function ($scope, $state, $stateParams, 
                    $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $cache,
                    Data, Sinodo, Presbiterio, Igreja, PontoDePregacao) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.pontosdepregacao.buscar()' : $stateParams.back + '()';
    ...
    Sinodo.getAll('', '', '', 'nome,asc').then(function(r) {
        ...
        Presbiterio.getAll('', '', '', 'nome,asc').then(function(r) {
            ...
            Igreja.getAll('', '', '', 'nome,asc').then(function(r) {
                if(r.total > 0) {
                    for(var k in r.datas) {
                        r.datas[k]['show'] = true;
                    }
    ...
    $scope.filterIgrejas = function(v) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_pontosdepregacao';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.filterIgrejas($scope.presbiterio);

        $scope.list = [];
        PontoDePregacao.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
            ...
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    ...
                    if(r.datas[k].igreja != null) {
                        var i = $scope.listIgrejas.filter(x => x.id == r.datas[k].igreja);
                        if(i.length > 0) {
                            r.datas[k]['igreja_str'] = i[0].nome;
                        }
                    }
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            PontoDePregacao.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = PontoDePregacao.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = PontoDePregacao.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            PontoDePregacao.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            PontoDePregacao.remove(d.id).then(function() {
                $timeout(function() {
