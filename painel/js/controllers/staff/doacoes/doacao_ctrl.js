angular.module('SmartChurchPanel').controller('DoacaoCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout,
                    Doacao) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.doacoes.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Doação';
    $scope.objList = 'Doações';
    ...
    /* search */
    $scope.storage_cache_name = 'search_doacoes';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Doacao.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy).then(function(r) {
            if (r.datas.length > 0) {
                $scope.list = r.datas;
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        Doacao.getMe($scope.id).then(function(r) {
            $scope.data = r;
            $scope.load();
        }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Doacao.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Doacao.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Doacao.remove(d.id).then(function() {
                $timeout(function() {
