angular.module('SmartChurchPanel').controller('PresbiterioCtrl', function ($scope, $state, $stateParams, $localstorage, 
                    $notifications, $rootScope, $dialogs, $timeout, $cache,
                    Data, Sinodo, Presbiterio) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.presbiterios.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Presbitério';
    $scope.objList = 'Presbitérios';
    ...
    $scope.localPerms = {
        add: 'PresbiterioSave',
        edit: 'PresbiterioSave',
        changeStat: 'PresbiterioBlock',
        remove: 'PresbiterioRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_presbiterios';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.stat != '' || $scope.sinodo != '');
    };
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Presbiterio.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, $scope.sinodo).then(function(r) {
            if (r.datas.length > 0) {
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Presbiterio.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Presbiterio.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Presbiterio.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Presbiterio.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Presbiterio.remove(d.id).then(function() {
                $timeout(function() {
