angular.module('SmartChurchPanel').controller('ProfissaoCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout,
                    Profissao) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.profissoes.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Profissão';
    $scope.objList = 'Profissões';
    ...
    $scope.localPerms = {
        add: 'ProfissaoSave',
        edit: 'ProfissaoSave',
        remove: 'ProfissaoRemove'
    };
    
    /* search */
    $scope.storage_cache_name = 'search_profissoes';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Profissao.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy).then(function(r) {
            if (r.datas.length > 0) {
                $scope.list = r.datas;
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        Profissao.getMe($scope.id).then(function(r) {
            $scope.data = r;
            $scope.load();
        }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Profissao.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Profissao.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Profissao.remove(d.id).then(function() {
                $timeout(function() {
