angular.module('SmartChurchPanel').controller('PerfilCtrl', function($scope, $state, $stateParams, $localstorage, $dialogs, $rootScope, $timeout, $notifications, User, Permissao, Perfil) { 
    
    /* config */
    $scope.opTitle = $stateParams.title;
    $scope.search = $stateParams.search;
    $scope.data = $stateParams.data;
    $scope.perms = $stateParams.perms;
    $scope.back = ($stateParams.back == '') ? 'dashboard.perfis.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Perfil';
    $scope.objList = 'Perfis';
    $scope.isCreate = false;

    /* verificação de permissão do state */
    if(!$scope.USER.doIHaveAccess($scope.user, $scope.perms)) {
        $state.go('SmartChurchPanel.sempermissao');
    }
    
    $scope.localPerms = {
        add: 'PerfilSave',
        edit: 'PerfilSave',
        remove: 'PerfilSave'
    };
    
    /* common */
    $scope.listPermissoes = [];
    Permissao.getAll('', '', '', 'ordem,asc').then(function(r) {
        if(r.total > 0) {
            $scope.listPermissoes = r.datas;
        }
        
        $scope.$broadcast('preLoad');
    }, function(e) { console.log(e); $scope.testError(e); });
    
    
    /* search */
    $scope.storage_cache_name = 'search_perfil';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    $scope.hasNext = false;
    $scope.toPrev = function() {
        $scope.page--;
        var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
        if (!$.isEmptyObject(prev_search)) {
            prev_search.page = $scope.page;
            $localstorage.setObject($scope.storage_cache_name, prev_search);
        }
        $scope.doSearch();
    };
    $scope.toNext = function() {
        $scope.page++;
        var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
        if (!$.isEmptyObject(prev_search)) {
            prev_search.page = $scope.page;
            $localstorage.setObject($scope.storage_cache_name, prev_search);
        }
        $scope.doSearch();
    };
    $scope.list = [];
    $scope.createSearchObject = function(only_non_existent) {
        if(only_non_existent != undefined && only_non_existent == true) {
            var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
            if (!$.isEmptyObject(prev_search)) {
                return;
            }
        }
        
        $localstorage.setObject($scope.storage_cache_name, {
            page: $scope.page,
            pageSize: $scope.pageSize,
            searchBy: '',
            orderBy: 'time_cad,desc'
        });
    };
    $scope.createSearchObject(true);
    $scope.clear = function() {
        $scope.searchBy = '';
        $scope.page = 1;
        $scope.createSearchObject();
        $scope.orderField = 'time_cad';
        $scope.orderOrientation = 'desc';
        $scope.doSearch();
        $scope.clearMarkList();
    };
    $scope.orderField = 'time_cad';
    $scope.orderOrientation = 'desc';
    $scope.doSort = function() {
        if($scope.orderField != '') {
            $scope.orderBy = $scope.orderField + ',' + $scope.orderOrientation;
            var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
            if (!$.isEmptyObject(prev_search)) {
                prev_search.orderBy = $scope.orderBy;
                $localstorage.setObject($scope.storage_cache_name, prev_search);
            }
        }

        $scope.doSearch();
    };
    $scope.orderBy = 'time_cad,desc';
    $scope.total = 0;
    $scope.getPreviousOrdering = function() {
        var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
        if (!$.isEmptyObject(prev_search)) {
            $scope.orderBy = prev_search.orderBy;
            var o_v = $scope.orderBy.split(',');
            $scope.orderField = o_v[0];
            $scope.orderOrientation = o_v[1];
        }
    };
    $scope.getPreviousOrdering();
    $scope.doSearch = function(is_new) {

        if (is_new != undefined && is_new == true) {
            
            var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
            if (!$.isEmptyObject(prev_search)) {
                if(prev_search.searchBy != $scope.searchBy) {
                    $scope.page = 1; 
                    $scope.pageSize = '10';
                }
            }
            
            $localstorage.setObject($scope.storage_cache_name, {
                page: $scope.page,
                pageSize: $scope.pageSize,
                searchBy: $scope.searchBy,
                orderBy: $scope.orderBy
            });
        } else {
            var prev_search = $localstorage.getObject($scope.storage_cache_name, {});
            if (!$.isEmptyObject(prev_search)) {
                $scope.page = prev_search.page;
                $scope.pageSize = prev_search.pageSize;
                $scope.searchBy = prev_search.searchBy;
                $scope.orderBy = prev_search.orderBy;
                var o_v = $scope.orderBy.split(',');
                $scope.orderField = o_v[0];
                $scope.orderOrientation = o_v[1];
            }
        }

        $scope.list = [];
        Perfil.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy).then(function(r) {
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    r.datas[k]['time_cad_str'] = moment(r.datas[k].time_cad).format('DD/MM/YYYY');
                    r.datas[k]['last_mod_str'] = moment(r.datas[k].last_mod).format('DD/MM/YYYY HH:mm');
                    r.datas[k]['selected'] = false;
                }
                
                $scope.list = r.datas;
                $scope.hasNext = (r.total > ((($scope.page - 1) * parseInt($scope.pageSize)) + r.datas.length));
                $scope.hasPrev = ($scope.page > 1);
                $scope.total = r.total;
                
                $rootScope.list = $scope.list;
            } else {
                $scope.hasNext = false;
                $scope.hasPrev = false;
                $scope.total = r.total;
                $notifications.info(':(', 'Sem dados para exibir');
            }
        }, function(e) {
            $notifications.err(e);
        });
    };
    if ($scope.search) {
        if ($scope.markList) {
            $scope.clearMarkList();
        }
        
        $scope.$on('preLoad', function() {
            $scope.doSearch();
        });
    }
    
    
    /* form */
    $scope.dataFrm = {
        isValid: false,
        data: {
            id: { value: '', notEmpty: false, valid: true, StringfyFrom: '' },
            nome: { value: '', notEmpty: true, valid: true, StringfyFrom: '' },
            descricao: { value: '', notEmpty: false, valid: true, StringfyFrom: '' },
            staff_only: { value: false, notEmpty: false, valid: true, StringfyFrom: '' },
            permissoes: { value: [], notEmpty: false, valid: true, StringfyFrom: '' }
        },
        toSend: {
            id: '',
            nome: '',
            descricao: '',
            staff_only: false,
            permissoes: []
        },
        validate: function() {
            var valid = true;
            for (var k in $scope.dataFrm.data) {
                if ($scope.dataFrm.data[k].notEmpty) {
                    if ($scope.dataFrm.data[k].value == '') {
                        valid = false;
                        $scope.dataFrm.data[k].valid = false;
                    }
                }
            }
            
            $scope.dataFrm.isValid = valid;
            
            $scope.$apply();
        },
        prepare: function() {
            for (var k in $scope.dataFrm.data) {
                if ($scope.dataFrm.data[k].StringfyFrom != '') {
                    $scope.dataFrm.data[k].value = JSON.stringify($scope.dataFrm.data[$scope.dataFrm.data[k].StringfyFrom].value);
                }
                if ($scope.dataFrm.toSend.hasOwnProperty(k)) {
                    $scope.dataFrm.toSend[k] = $scope.dataFrm.data[k].value;
                }
            }
            
            $scope.dataFrm.toSend.permissoes = [];
            for(var k in $scope.dataFrm.data.permissoes.value) {
                for(var j in $scope.dataFrm.data.permissoes.value[k].subgrupo) {
                    for(var l in $scope.dataFrm.data.permissoes.value[k].subgrupo[j].permissoes) {
                        $scope.dataFrm.toSend.permissoes.push($scope.dataFrm.data.permissoes.value[k].subgrupo[j].permissoes[l]);
                    }
                }
            }
        }
    };
    
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Perfil.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { $state.go($scope.back.parseState().state, $scope.back.parseState().params); });
        });
    }
    
    $scope.preparePermissions = function(perms) {
        var checked = false;
        $scope.dataFrm.data.permissoes.value = [];
        var i, j;
        for(var k in $scope.listPermissoes) {
            var greater = ($scope.listPermissoes[k].grupo.includes(':'));
            if(!greater) {
                i = $scope.dataFrm.data.permissoes.value.findIndex(x => x.grupo == $scope.listPermissoes[k].grupo);
                if(i < 0) {
                    // grupo novo. Insira
                    $scope.dataFrm.data.permissoes.value.push({
                        show: false,
                        grupo: $scope.listPermissoes[k].grupo,
                        subgrupo: [{
                            nome: '',
                            permissoes: []
                        }]
                    });
                    i = $scope.dataFrm.data.permissoes.value.length - 1;
                    j = 0;
                }
            }
            else {
                var grupo = $scope.listPermissoes[k].grupo.split(':');
                i = $scope.dataFrm.data.permissoes.value.findIndex(x => x.grupo == grupo[0].trim());
                if(i < 0) {
                    // grupo novo. Insira
                    $scope.dataFrm.data.permissoes.value.push({
                        show: false,
                        grupo: grupo[0].trim(),
                        subgrupo: []
                    });
                    i = $scope.dataFrm.data.permissoes.value.length - 1;
                    
                    $scope.dataFrm.data.permissoes.value[i].subgrupo.push({
                        nome: grupo[1].trim(),
                        permissoes: []
                    });
                    j = $scope.dataFrm.data.permissoes.value[i].subgrupo.length - 1;
                }
                else {
                    j = $scope.dataFrm.data.permissoes.value[i].subgrupo.findIndex(x => x.nome == grupo[1].trim());
                    if(j < 0) {
                        $scope.dataFrm.data.permissoes.value[i].subgrupo.push({
                            show: false,
                            nome: grupo[1].trim(),
                            permissoes: []
                        });
                        j = $scope.dataFrm.data.permissoes.value[i].subgrupo.length - 1;
                    }
                }
            }
            
            if(perms != undefined) {
                var finded = perms.filter(x => x.id == $scope.listPermissoes[k].id);
                checked = (finded.length > 0);
            }
            
            $scope.dataFrm.data.permissoes.value[i].subgrupo[j].permissoes.push({
                id: $scope.listPermissoes[k].id,
                label: $scope.listPermissoes[k].descricao,
                checked: checked
            });
        }
        
    };
    
    $scope.toggleSubGrupoPermissions = function(g) {
        g.show = !g.show;
    };
    
    $scope.load = function() {
        for (var k in $scope.data) {
            if ($scope.dataFrm.data.hasOwnProperty(k)) {
                $scope.dataFrm.data[k].value = $scope.data[k];
            }
        }

        Perfil.getPermissoes($scope.data.id).then(function(r) {
            if(r.total > 0) {
                $scope.preparePermissions(r.datas);
            }
            else {
                $scope.preparePermissions();
            }
        }, function(e) {
            $scope.preparePermissions();
        });
    };
    
    if(!$scope.data && !$scope.search && !$scope.id) {
        // adição
        $scope.isCreate = true;
        $scope.$on('preLoad', function() {
            $scope.preparePermissions();
        });
    }
    
    
    /* save */
    $('#dataFrm').validate({
        submit: {
            settings: {
                inputContainer: '.form-group',
                errorListClass: 'form-control-error',
                errorClass: 'has-danger'
            },
            callback: {
                onSubmit: function(node, formData) {
                    $scope.doSave();
                },
                onError: function (node, globalError) {
                    $notifications.err("Há campos incorretos!");
                }
            }
        }
    });
    
    $scope.doSave = function() {
        $scope.dataFrm.validate();
        if ($scope.dataFrm.isValid) {
            $scope.dataFrm.prepare();
            var promise = {};
            if($scope.isCreate)
            {
                promise = Perfil.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Perfil.edit($scope.id, $scope.dataFrm.toSend);
            }
            
            promise.then(function(r) {
                $dialogs.onSave().then(function() {
                    $state.reload();
                }, function() {});
            }, function(e) {
                $notifications.err(e);
            });
        } else {
            $notifications.err("Há campos incorretos!");
        }
    };
    
    /* remove */
    $scope.remove = function(d) {
        
        $dialogs.beforeRemove().then(function() {
            Perfil.remove(d.id).then(function() {
                $timeout(function() {
                    $dialogs.onRemove().then(function() {
                        $state.reload();
                    }, function() {});
                }, 100);
            }, function(e) {
                $notifications.err(e);
            });
        }, function() {  });
    };

    $scope.removeSelected = function() {

        var ids = [];
        for (var k in $scope.markList) {
            ids.push($scope.markList[k].id);
        }

        $dialogs.beforeRemove().then(function() {
            Perfil.removeAll({
                ids: ids
            }).then(function() {
                $timeout(function() {
                    $dialogs.onRemove().then(function() {
                        $state.reload();
                    }, function() {});
                }, 100);
            }, function(e) {
                $notifications.err(e);
            });
        }, function() {});
    };
    
});


