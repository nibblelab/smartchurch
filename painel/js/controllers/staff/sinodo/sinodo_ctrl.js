angular.module('SmartChurchPanel').controller('SinodoCtrl', function ($scope, $state, $stateParams, $localstorage, 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.sinodos.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Sínodo';
    $scope.objList = 'Sínodos';
    ...
    /* search */
    $scope.storage_cache_name = 'search_sinodos';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Sinodo.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat).then(function(r) {
            if (r.datas.length > 0) {
    ...
    $scope.dataFrm = {
        ...
        prepare: function() {
            ...
            $scope.dataFrm.toSend.nacional = $scope.listSupremo[0].id;
        }
    };
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Sinodo.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Sinodo.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Sinodo.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Sinodo.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Sinodo.remove(d.id).then(function() {
                $timeout(function() {
