angular.module('SmartChurchPanel').controller('NecessidadeCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout,
                    Necessidade) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.necessidadesespeciais.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Necessidade Especial';
    $scope.objList = 'Necessidades Especiais';
    ...
    $scope.localPerms = {
        add: 'NecessidadeSave',
        edit: 'NecessidadeSave',
        remove: 'NecessidadeRemove'
    };
    
    /* search */
    $scope.storage_cache_name = 'search_necessidadesespeciais';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Necessidade.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy).then(function(r) {
            if (r.datas.length > 0) {
                $scope.list = r.datas;
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        Necessidade.getMe($scope.id).then(function(r) {
            $scope.data = r;
            $scope.load();
        }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Necessidade.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Necessidade.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Necessidade.remove(d.id).then(function() {
                $timeout(function() {
