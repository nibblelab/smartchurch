angular.module('SmartChurchPanel').controller('CongregacaoCtrl', function ($scope, $state, $stateParams, $localstorage, 
                    $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $cache,
                    Data, Sinodo, Presbiterio, Igreja, Congregacao) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.congregacoes.buscar()' : $stateParams.back + '()';
    ...
    $scope.localPerms = {
        add: 'CongregacaoSave',
        edit: 'CongregacaoSave',
        changeStat: 'CongregacaoBlock',
        remove: 'CongregacaoRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_congregacoes';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Congregacao.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Congregacao.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Congregacao.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Congregacao.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Congregacao.remove(d.id).then(function() {
                $timeout(function() {
