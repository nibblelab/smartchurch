angular.module('SmartChurchPanel').controller('CargoCtrl', function ($scope, $state, $stateParams, $localstorage, 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.cargos.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Cargo';
    $scope.objList = 'Cargos';
    ...
    /* search */
    $scope.storage_cache_name = 'search_cargos';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.instancia != '' || $scope.perfil != '');
    };
    $scope.doSearch = function(is_new) {
        ...
        if (is_new != undefined && is_new == true) {
            ...
            if (!$.isEmptyObject(prev_search)) {
                ...
                $scope.instancia = prev_search.instancia;
                $scope.perfil = prev_search.perfil;
                $scope.orderBy = prev_search.orderBy;
                var o_v = $scope.orderBy.split(',');
                $scope.orderField = o_v[0];
                $scope.orderOrientation = o_v[1];
        ...
        $scope.list = [];
        Cargo.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.instancia, $scope.perfil).then(function(r) {
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    ...
                    r.datas[k]['perfil_str'] = '';
                    var pf = $scope.listPerfis.filter(x => x.id == r.datas[k].perfil);
                    if(pf.length > 0) {
                        r.datas[k]['perfil_str'] = pf[0].nome;
                    }
                    
                    r.datas[k]['instancia_str'] = '';
                    var it = $scope.listInstancias.filter(x => x.value == r.datas[k].instancia);
                    if(it.length > 0) {
                        r.datas[k]['instancia_str'] = it[0].label;
                    }
    ...
    $scope.dataFrm = {
        ...
        data: {
            ...
            instancia: { value: '', notEmpty: true, valid: true, StringfyFrom: '' }
        },
        toSend: {
            id: '',
            perfil: '',
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Cargo.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Cargo.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Cargo.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Cargo.remove(d.id).then(function() {
                $timeout(function() {
