angular.module('SmartChurchPanel').controller('FederacaodaSinodalCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $context, $cache,
                    Sociedade, Federacao) { 
    ...
    $scope.doSearch = function(is_new) {
        ...
        Federacao.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
                            '', '', $scope.sinodal, $scope.sinodalData.reference).then(function(r) {
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    ...
                    r.datas[k]['sociedades'] =  $scope.listSociedades.filter(x => x.federacao == r.datas[k].id).length;
