angular.module('SmartChurchPanel').controller('SecretarioDaSinodalCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context, $cache,
                    Secretaria, Socio, Perfil, SysConfig, Secretario) { 
    ...
    $scope.sref = {
        back_level2: 'SmartChurchPanel.sinodal.secretarias.buscar()',
        back: ($stateParams.back == '') ? 'SmartChurchPanel.sinodal.secretarias.secretarios.buscar' : $stateParams.back,
        backParams: { secretaria: $scope.secretaria },
        add: "SmartChurchPanel.sinodal.secretarias.secretarios.adicionar",
        addParams: { back: 'SmartChurchPanel.sinodal.secretarias.secretarios.buscar', secretaria: $scope.secretaria },
        edit: "SmartChurchPanel.sinodal.secretarias.secretarios.editar",
        editParams: { id: '', title: 'Editando', back: 'SmartChurchPanel.sinodal.secretarias.secretarios.buscar', secretaria: $scope.secretaria }
    };
    $scope.id = $stateParams.id;
    $scope.objForm = 'Secretário';
    $scope.objList = 'Secretários';
    $scope.context = $context.getActiveContext();
    $scope.sinodal = $scope.context.id;
    $scope.sinodalData = $rootScope.USER.getContextByKeyAndId($scope.context.key, $scope.context.id);
    ...
    $scope.localPerms = {
        add: 'SecretariaSinodalPessoa',
        edit: 'SecretariaSinodalPessoa',
        changeStat: 'SecretariaSinodalPessoa',
        remove: 'SecretariaSinodalPessoa'
    };
    
    /* common */
    $scope.listTipos = $cache.get().tipos_secretario;
    $scope.listStatus = $cache.get().status;
    $scope.listSexos = $cache.get().sexo;
    $scope.listEscolaridade = $cache.get().escolaridade;
    $scope.listEstadoCivil = $cache.get().estado_civil;
    ...
    Secretaria.getMe($scope.secretaria).then(function(r) {
        ...
        Socio.getAll('', '', '', '', '', '', '', Status.ATIVO, '', '', '', true, 
                        '', '', '', '', '', '', '', $scope.sinodal).then(function(r) {
            if(r.total > 0) {
    ...
    }, function(e) { console.log(e); $state.go('SmartChurchPanel.sinodal.secretarias.buscar()'); });
    
    /* search */
    $scope.storage_cache_name = 'search_secretariosdasinodal';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
