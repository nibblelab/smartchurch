angular.module('SmartChurchPanel').controller('SociedadedaSinodalCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $context, $cache,
                    Templo, Socio, Federacao, Sociedade) { 
    ...
    Federacao.getAllForSinodal($scope.sinodal, $scope.sinodalData.reference).then(function(r) {
        ...
        Templo.getAll('', '', '', '', '', '', $scope.sinodalData.sinodo).then(function(r) {
            if(r.total > 0) {
                $scope.listTemplos = r.datas;
            }

            Socio.getAllForSinodal($scope.sinodal).then(function(r) {
                if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_sociedadesdasinodal';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.stat != '' || $scope.igreja != '' || $scope.federacao != '');
    };
    $scope.doSearch = function(is_new) {
        ...
        Sociedade.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
                            $scope.igreja, $scope.federacao, $scope.sinodal, '', $scope.sinodalData.reference).then(function(r) {
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    ...
                    r.datas[k]['igreja_nome'] = '';
                    var igreja = $scope.listTemplos.find(x => x.id == r.datas[k].igreja);
                    if(igreja != undefined) {
                        r.datas[k]['igreja_nome'] = igreja.nome;
                    }
                    
                    r.datas[k]['federacao_nome'] = '';
                    var fed = $scope.listFederacoes.find(x => x.id == r.datas[k].federacao);
                    if(fed != undefined) {
                        r.datas[k]['federacao_nome'] = fed.sigla;
                    }
                    
                    r.datas[k]['socios'] = $scope.listSocios.filter(x => x.sociedade == r.datas[k].id).length;
