angular.module('SmartChurchPanel').controller('OficialSinodalCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
    ...
    $scope.backDiretorias = 'SmartChurchPanel.sinodal.diretorias.buscar()';
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.sinodal.diretorias.oficiais.buscar({diretoria: "'+$scope.diretoria+'"})' : $stateParams.back + '({diretoria: "'+$scope.diretoria+'"})';
    ...
    $scope.navStates = {
        add: 'SmartChurchPanel.sinodal.diretorias.oficiais.adicionar',
        edit: 'SmartChurchPanel.sinodal.diretorias.oficiais.editar',
        search: 'SmartChurchPanel.sinodal.diretorias.oficiais.buscar'
    };
    
    $scope.localPerms = {
        add: 'DiretoriaSinodalOficiais',
        edit: 'DiretoriaSinodalOficiais',
        changeStat: 'DiretoriaSinodalOficiais',
        remove: 'DiretoriaSinodalOficiais'
    };
    ...
    Diretoria.getMe($scope.diretoria).then(function(r) {
        ...
        Socio.getAll('', '', '', '', '', '', '', Status.ATIVO, '', '', '', true, 
            ...
            Cargo.getAllForSinodal().then(function(r) {
                if(r.total > 0) {
    ...
    }, function(e) { console.log(e); $state.go('SmartChurchPanel.sinodal.diretorias.buscar()'); });
    
    /* search */
    $scope.storage_cache_name = 'search_oficiaisdasinodal';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.dataFrm = {
        ...
        data: {
            ...
            demoted: { value: false, notEmpty: false, valid: true, StringfyFrom: '' }
        },
        toSend: {
            id: '',
            pessoa: '',
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Oficial.createForSinodal($scope.dataFrm.toSend);
            }
            else
            {
