angular.module('SmartChurchPanel').controller('PessoadaSinodalCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $context, $cache,
                    Templo, Sociedade, Federacao, Socio) { 
    ...
    Federacao.getAllForSinodal($scope.sinodal, $scope.sinodalData.reference).then(function(r) {
        ...
        Templo.getAll('', '', '', '', '', '', $scope.sinodalData.sinodo).then(function(r) {
            ...
            Sociedade.getAll('', '', '', '', '', '', '', '', $scope.sinodal, '', $scope.sinodalData.reference).then(function(r) {
                if(r.total > 0) {
                    for(var k in r.datas) {
                        ...
                        if(fd != undefined) {
                            r.datas[k]['federacao_nome'] = fd.sigla;
                        }
                    }
    ...
    /* search */
    $scope.storage_cache_name = 'search_pessoasdasinodal';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.stat != '' || $scope.apenas_diretoria || $scope.sociedade != '' || 
                $scope.federacao != '' || $scope.sexo != '' || $scope.escolaridade != '' ||
    ...
    $scope.doSearch = function(is_new) {
        ...
        Socio.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.sociedade, '', $scope.stat, '', 
                            ...
                            $scope.tem_filhos, $scope.sem_filhos, '', $scope.federacao, $scope.sinodal).then(function(r) {
            if (r.datas.length > 0) {
                for (var k in r.datas) {
                    ...
                    if(sc != undefined) {
                        ...
                        r.datas[k]['federacao_nome'] = sc.federacao_nome;
                    }
