angular.module('SmartChurchPanel').controller('AgendaDaSociedadeCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $context,
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.sociedade.agenda.buscar()' : $stateParams.back + '()';
    ...
    $scope.igreja = $scope.sociedadeData.igreja;
    ...
    $scope.localPerms = {
        add: 'AgendaSociedadeSave',
        edit: 'AgendaSociedadeSave',
        changeStat: 'AgendaSociedadeBlock',
        remove: 'AgendaSociedadeRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_agendadasociedade';
    $scope.searchBy = '';
    $scope.list = [];
    ...
    $scope.clear = function() {
        ...
        $scope.termino = '';
        $scope.stat = '';
        $scope.responsavel = $scope.sociedade;
        ...
        if($scope.list.length > 0) {
            $scope.calendarObj = $('#calendar').fullCalendar({
                ...
                eventRender: function(event, element) {
                    ...
                    var opts = '';
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaSociedadeBlock')) {
                        if(event.stat == 'ATV')
                        {
                    ...
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaSociedadeSave')) {
                        opts += '&nbsp;<button type="button" class="btn btn-warning btn-xs calendar-edit" data-popover-id="'+popover_id+'"> '+
                                    '    <i class="fas fa-edit"></i>'+
                                '</button>'
                            ;
                    }
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaSociedadeRemove')) {
    ...
    $scope.toEdit = function(id) {
        $state.go('SmartChurchPanel.sociedade.agenda.editar', {id: id, title: 'Editando', back: 'SmartChurchPanel.sociedade.agenda.buscar'});
    };
