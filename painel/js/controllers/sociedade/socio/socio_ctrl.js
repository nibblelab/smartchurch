angular.module('SmartChurchPanel').controller('SocioCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Data, MembroDaIgreja, Socio) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.sociedade.socios.buscar()' : $stateParams.back + '()';
    ...
    Data.getAll().then(function(r) {
        ...
        // veja se a sociedade tem limite de idade e caso tenha, aplique o filtro nos membros da igreja
        $scope.is_crianca = ($scope.sociedadeData.idade.crianca) ? true : '';
        $scope.is_adolescente = ($scope.sociedadeData.idade.adolescente) ? true : '';
        $scope.is_jovem = ($scope.sociedadeData.idade.jovem) ? true : '';
            
        MembroDaIgreja.getAll('', '', '', '', '', '', $scope.sociedadeData.igreja, '', '', '', 
                            $scope.is_crianca, $scope.is_adolescente, $scope.is_jovem).then(function(r) {
            if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_socios';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.stat != '' || $scope.sexo != '' || $scope.escolaridade != '' || 
                                $scope.estado_civil != '' || $scope.cooperador || $scope.apenas_socios || 
    ...
    $scope.doSearch = function(is_new) {
        ...
        if (is_new != undefined && is_new == true) {
            ...
            $localstorage.setObject($scope.storage_cache_name, {
                ...
                apenas_socios: $scope.apenas_socios,
            ...
            if (!$.isEmptyObject(prev_search)) {
                ...
                $scope.apenas_socios = prev_search.apenas_socios;
        ...
        Socio.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.sociedade, '', $scope.stat, '', '',
                            $scope.cooperador, $scope.apenas_socios, $scope.sexo, $scope.estado_civil, $scope.escolaridade, 
