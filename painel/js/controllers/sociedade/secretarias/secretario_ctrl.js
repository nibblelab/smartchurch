angular.module('SmartChurchPanel').controller('SecretarioDaSociedadeCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Data, Secretaria, Socio, Perfil, SysConfig, Secretario) { 
    ...
    $scope.sref = {
        back_level2: 'SmartChurchPanel.sociedade.secretarias.buscar()',
        back: ($stateParams.back == '') ? 'SmartChurchPanel.sociedade.secretarias.secretarios.buscar' : $stateParams.back,
        backParams: { secretaria: $scope.secretaria },
        add: "SmartChurchPanel.sociedade.secretarias.secretarios.adicionar",
        addParams: { back: 'SmartChurchPanel.sociedade.secretarias.secretarios.buscar', secretaria: $scope.secretaria },
        edit: "SmartChurchPanel.sociedade.secretarias.secretarios.editar",
        editParams: { id: '', title: 'Editando', back: 'SmartChurchPanel.sociedade.secretarias.secretarios.buscar', secretaria: $scope.secretaria }
    };
    $scope.id = $stateParams.id;
    $scope.objForm = 'Secretário';
    $scope.objList = 'Secretários';
    $scope.context = $context.getActiveContext();
    $scope.sociedade = $scope.context.id;
    $scope.sociedadeData = $rootScope.USER.getContextByKeyAndId($scope.context.key, $scope.context.id);
    ...
    $scope.localPerms = {
        add: 'SecretariaSociedadePessoa',
        edit: 'SecretariaSociedadePessoa',
        changeStat: 'SecretariaSociedadePessoa',
        remove: 'SecretariaSociedadePessoa'
    };
    ...
    Data.getAll().then(function(r) {
        ...
        Secretaria.getMe($scope.secretaria).then(function(r) {
            ...
            Socio.getAll('', '', '', '', '', $scope.sociedade).then(function(r) {
                if(r.total > 0) {
        ...
        }, function(e) { console.log(e); $state.go('SmartChurchPanel.sociedade.secretarias.buscar()'); });
    }, function(e) { console.log(e); $scope.testError(e); });
    
    /* search */
    $scope.storage_cache_name = 'search_secretariosdasociedade';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
