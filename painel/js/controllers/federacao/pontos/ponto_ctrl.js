angular.module('SmartChurchPanel').controller('PontodaFederacaoCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $context,
                    Data, Sociedade, Igreja, PontoDePregacao) { 
    ...
    Data.getAll().then(function(r) {
        ...
        Sociedade.getAllAtivosForFederacao($scope.federacao, $scope.federacaoData.reference).then(function(r) {
            if(r.total > 0) {
                $scope.listSociedades = r.datas;
            }
            
            Igreja.getAll('', '', '', '', '', '', $scope.federacaoData.sinodo, $scope.federacaoData.presbiterio).then(function(r) {
                if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_pontosdafederacao';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        PontoDePregacao.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
                            $scope.federacaoData.sinodo, $scope.federacaoData.presbiterio).then(function(r) {
            if (r.datas.length > 0) {
