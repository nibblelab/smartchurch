angular.module('SmartChurchPanel').controller('AgendaDoPastorCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $context,
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.pastor.agenda.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Agenda';
    $scope.objList = 'Agenda';
    $scope.igreja = $context.getPastorContext();
    $scope.igrejaData = $rootScope.USER.getContextByKeyAndId(Contexts.PASTORES, $scope.igreja);
    ...
    $scope.localPerms = {
        add: 'AgendaPastorSave',
        edit: 'AgendaPastorSave',
        changeStat: 'AgendaPastorBlock',
        remove: 'AgendaPastorRemove'
    };
    ...
    $scope.pastorId = $rootScope.USER.getOficialId('pastor', $scope.igreja);
    Data.getAll().then(function(r) {
        $scope.listStatus = r.status;
        $scope.listUFs = r.ufs;
        $scope.listCidades = r.cidades;
        TagAgenda.getAll('', '', '', '', '', '', 'geral,pastor').then(function(r) {
            if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_agendadopastor';
    $scope.searchBy = '';
    $scope.list = [];
    ...
    $scope.clear = function() {
        ...
        $scope.termino = '';
        $scope.stat = '';
        $scope.responsavel = $scope.pastorId;
        ...
        if($scope.list.length > 0) {
            $scope.calendarObj = $('#calendar').fullCalendar({
                ...
                eventRender: function(event, element) {
                    ...
                    var opts = '';
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaPastorBlock')) {
                        if(event.stat == 'ATV')
                        {
                    ...
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaPastorSave')) {
                        opts += '&nbsp;<button type="button" class="btn btn-warning btn-xs calendar-edit" data-popover-id="'+popover_id+'"> '+
                                    '    <i class="fas fa-edit"></i>'+
                                '</button>'
                            ;
                    }
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaPastorRemove')) {
    ...
    $scope.toEdit = function(id) {
        $state.go('SmartChurchPanel.pastor.agenda.editar', {id: id, title: 'Editando', back: 'SmartChurchPanel.pastor.agenda.buscar'});
    };
