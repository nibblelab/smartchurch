angular.module('SmartChurchPanel').controller('EvangelistaDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Data, MembroDaIgreja, SysConfig, Evangelista) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.evangelistas.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Evangelista';
    $scope.objList = 'Evangelistas';
    ...
    $scope.localPerms = {
        add: 'EvangelistaIgrejaSave',
        edit: 'EvangelistaIgrejaSave',
        changeStat: 'EvangelistaIgrejaBlock',
        remove: 'EvangelistaIgrejaRemove'
    };
    ...
    Data.getAll().then(function(r) {
        ...
        MembroDaIgreja.getAll('', '', '', '', '', '', $scope.igreja, true).then(function(r) {
            if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_evangelistasdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Evangelista.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    $scope.selectDefaultPerfil = function() {
        $scope.dataFrm.data.perfil.value = $scope.listConfigs[0].perfil_evangelista;
    };
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Evangelista.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Evangelista.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Evangelista.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Evangelista.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Evangelista.remove(d.id).then(function() {
                $timeout(function() {
