angular.module('SmartChurchPanel').controller('ParticipanteMinisterioCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Ministerio, MembroDaIgreja, Perfil, SysConfig, Servo) { 
    
    /* config */
    $scope.ministerio = $stateParams.ministerio;
    $scope.opTitle = $stateParams.title;
    $scope.search = $stateParams.search;
    $scope.data = $stateParams.data;
    $scope.perms = $stateParams.perms;
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.ministerios.participantes.buscar({ministerio: "'+$scope.ministerio+'"})' : $stateParams.back + '({ministerio: "'+$scope.ministerio+'"})';
    ...
    $scope.localPerms = {
        add: 'MinisterioIgrejaPessoa',
        edit: 'MinisterioIgrejaPessoa',
        changeStat: 'MinisterioIgrejaPessoa',
        remove: 'MinisterioIgrejaPessoa'
    };
    
    /* common */
    $scope.ministerioData = { nome: '' };
    $scope.listMembros = [];
    $scope.listPerfis = [];
    $scope.listConfigs = [];
    Ministerio.getMe($scope.ministerio).then(function(r) {
        $scope.ministerioData = r;
    ...
    }, function(e) { console.log(e); $state.go('SmartChurchPanel.igreja.ministeriosbuscar()'); });
    
    /* search */
    $scope.storage_cache_name = 'search_participantesdeministerio';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.isFilterUsed = function() {
        $scope.filterUsed = ($scope.stat != '' || $scope.admin);
    };
    $scope.doSearch = function(is_new) {
        ...
        Servo.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.ministerio, '', 
                        $scope.stat, $scope.admin).then(function(r) {
            if (r.datas.length > 0) {
    ...
    $scope.selectDefaultPerfil = function() {
        $scope.dataFrm.data.perfil.value = $scope.listConfigs[0].perfil_ministerio;
    };
