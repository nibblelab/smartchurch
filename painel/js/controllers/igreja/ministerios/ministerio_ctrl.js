angular.module('SmartChurchPanel').controller('MinisterioDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.ministerios.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Ministério';
    $scope.objList = 'Ministérios';
    ...
    $scope.localPerms = {
        add: 'MinisterioIgrejaSave',
        edit: 'MinisterioIgrejaSave',
        participante: 'MinisterioIgrejaPessoa',
        changeStat: 'MinisterioIgrejaBlock',
        remove: 'MinisterioIgrejaRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_ministeriosdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Ministerio.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Ministerio.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Ministerio.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Ministerio.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Ministerio.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Ministerio.remove(d.id).then(function() {
                $timeout(function() {
