angular.module('SmartChurchPanel').controller('JuntaDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $context, Junta) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.junta.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objList = 'Junta Diaconal';
    ...
    /* search */
    $scope.storage_cache_name = 'search_juntadaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Junta.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.igreja).then(function(r) {
            if (r.datas.length > 0) {
