angular.module('SmartChurchPanel').controller('PequenoGrupoDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.pequenosgrupos.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Pequeno Grupo';
    $scope.objList = 'Pequenos Grupos';
    ...
    $scope.localPerms = {
        add: 'PequenoGrupoIgrejaSave',
        edit: 'PequenoGrupoIgrejaSave',
        changeStat: 'PequenoGrupoIgrejaBlock',
        remove: 'PequenoGrupoIgrejaRemove',
        participante: 'PequenoGrupoIgrejaParticipantes'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_pequenosgruposdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        PequenoGrupo.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    $scope.dataFrm = {
        ...
        data: {
            ...
            reunioes: { value: '', notEmpty: false, valid: true, StringfyFrom: '' }
        },
        toSend: {
            id: '',
            igreja: $scope.igreja,
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            PequenoGrupo.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = PequenoGrupo.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = PequenoGrupo.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            PequenoGrupo.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            PequenoGrupo.remove(d.id).then(function() {
                $timeout(function() {
