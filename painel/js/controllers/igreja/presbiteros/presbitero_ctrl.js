angular.module('SmartChurchPanel').controller('PresbiteroDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Data, MembroDaIgreja, SysConfig, Presbitero) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.presbiteros.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Presbítero';
    $scope.objList = 'Presbíteros';
    ...
    $scope.localPerms = {
        add: 'PresbiteroIgrejaSave',
        edit: 'PresbiteroIgrejaSave',
        changeStat: 'PresbiteroIgrejaBlock',
        remove: 'PresbiteroIgrejaRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_presbiterosdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Presbitero.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    $scope.selectDefaultPerfil = function() {
        $scope.dataFrm.data.perfil.value = $scope.listConfigs[0].perfil_presbitero;
    };
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Presbitero.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Presbitero.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Presbitero.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Presbitero.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Presbitero.remove(d.id).then(function() {
                $timeout(function() {
