angular.module('SmartChurchPanel').controller('PontoDePregacaoIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $context,
                    Data, PontoDePregacaoIgreja) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.pontosdepregacao.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Ponto de Pregação';
    $scope.objList = 'Pontos de Pregação';
    ...
    $scope.localPerms = {
        add: 'PontoIgrejaSave',
        edit: 'PontoIgrejaSave',
        changeStat: 'PontoIgrejaBlock',
        remove: 'PontoIgrejaRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_pontosdepregacaoigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        PontoDePregacaoIgreja.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, $scope.igreja).then(function(r) {
            if (r.datas.length > 0) {
    ...
    $scope.dataFrm = {
        ...
        toSend: {
            id: '',
            sinodo: $scope.igrejaData.sinodo,
            presbiterio: $scope.igrejaData.presbiterio,
            igreja: $scope.igreja,
            nome: '',
            fundacao: '',
            telefone: '',
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            PontoDePregacaoIgreja.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = PontoDePregacaoIgreja.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = PontoDePregacaoIgreja.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            PontoDePregacaoIgreja.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            PontoDePregacaoIgreja.remove(d.id).then(function() {
                $timeout(function() {
    ...
    $scope.removeSelected = function() {
        ...
        $dialogs.beforeRemove().then(function() {
            PontoDePregacaoIgreja.removeAll({
                ids: ids
            }).then(function() {
                $timeout(function() {
                    $dialogs.onRemove().then(function() {
