angular.module('SmartChurchPanel').controller('DiaconoDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $context,
                    Data, MembroDaIgreja, SysConfig, Diacono) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.diaconos.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Diácono';
    $scope.objList = 'Diáconos';
    ...
    $scope.localPerms = {
        add: 'DiaconoIgrejaSave',
        edit: 'DiaconoIgrejaSave',
        changeStat: 'DiaconoIgrejaBlock',
        remove: 'DiaconoIgrejaRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_diaconosdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Diacono.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            Diacono.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Diacono.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Diacono.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Diacono.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Diacono.remove(d.id).then(function() {
                $timeout(function() {
