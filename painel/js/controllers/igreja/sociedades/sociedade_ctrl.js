angular.module('SmartChurchPanel').controller('SociedadeDaIgrejaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
                    $rootScope, $dialogs, $timeout, $ibge, $cep, $context, $cache,
                    Data, Nacional, Sinodal, Federacao, Sociedade) { 
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.igreja.sociedades.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Sociedade';
    $scope.objList = 'Sociedades';
    ...
    $scope.localPerms = {
        add: 'SociedadeIgrejaSave',
        edit: 'SociedadeIgrejaSave',
        changeStat: 'SociedadeIgrejaBlock',
        remove: 'SociedadeIgrejaRemove',
        socio: 'SociedadeIgrejaSocio'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_sociedadesdaigreja';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        Sociedade.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    $scope.onSelectSociedade = function() {
        Nacional.getAll('', '', '', '', '', '', $scope.dataFrm.data.reference.value).then(function(r) {
            if(r.total > 0) {
                $scope.listNacionais = r.datas;
                $scope.dataFrm.data.nacional.value = r.datas[0].id;
                Sinodal.getAll('', '', '', '', '', '', $scope.igrejaData.sinodo, 
                                    $scope.dataFrm.data.nacional.value, $scope.dataFrm.data.reference.value).then(function(r) {
                    if(r.total > 0) {
                        $scope.listSinodais = r.datas;
                        $scope.dataFrm.data.sinodal.value = r.datas[0].id;
                        Federacao.getAll('', '', '', '', '', '', $scope.igrejaData.sinodo, 
                                            $scope.igrejaData.presbiterio,
                                            $scope.dataFrm.data.sinodal.value, 
                                            $scope.dataFrm.data.reference.value).then(function(r) {
                            if(r.total > 0) {
                                $scope.listFederacoes = r.datas;
                                $scope.dataFrm.data.federacao.value = r.datas[0].id;
                            }
                        }, function() {});
                    }
                }, function() {});
    ...
    $scope.dataFrm = {
        ...
        data: {
            ...
            federacao: { value: '', notEmpty: false, valid: true, StringfyFrom: '' },
            sinodal: { value: '', notEmpty: false, valid: true, StringfyFrom: '' },
            nacional: { value: '', notEmpty: false, valid: true, StringfyFrom: '' },
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        Sociedade.getMe($scope.id).then(function(r) {
            $scope.data = r;
            $scope.load();
        }, function(e) { 
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = Sociedade.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = Sociedade.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            Sociedade.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            Sociedade.remove(d.id).then(function() {
                $timeout(function() {
