angular.module('SmartChurchPanel').controller('AgendaDoMinisterioCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $context,
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.ministerio.agenda.buscar()' : $stateParams.back + '()';
    ...
    $scope.igreja = $scope.ministerioData.igreja;
    ...
    $scope.localPerms = {
        add: 'AgendaMinisterioSave',
        edit: 'AgendaMinisterioSave',
        changeStat: 'AgendaMinisterioBlock',
        remove: 'AgendaMinisterioRemove'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_agendadoministerio';
    $scope.searchBy = '';
    $scope.list = [];
    ...
    $scope.clear = function() {
        ...
        $scope.termino = '';
        $scope.stat = '';
        $scope.responsavel = $scope.ministerio;
        ...
        if($scope.list.length > 0) {
            $scope.calendarObj = $('#calendar').fullCalendar({
                ...
                eventRender: function(event, element) {
                    ...
                    var opts = '';
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaMinisterioBlock')) {
                        if(event.stat == 'ATV')
                        {
                    ...
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaMinisterioSave')) {
                        opts += '&nbsp;<button type="button" class="btn btn-warning btn-xs calendar-edit" data-popover-id="'+popover_id+'"> '+
                                    '    <i class="fas fa-edit"></i>'+
                                '</button>'
                            ;
                    }
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaMinisterioRemove')) {
    ...
    $scope.toEdit = function(id) {
        $state.go('SmartChurchPanel.ministerioagenda.editar', {id: id, title: 'Editando', back: 'SmartChurchPanel.ministerioagenda.buscar'});
    };
