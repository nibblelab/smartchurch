angular.module('SmartChurchPanel').controller('AgendaDoEvangelistaCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, $rootScope, $dialogs, $timeout, 
                    $ibge, $cep, $context,
    ...
    $scope.back = ($stateParams.back == '') ? 'SmartChurchPanel.evangelista.agenda.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Agenda';
    $scope.objList = 'Agenda';
    $scope.igreja = $context.getEvangelistaContext();
    $scope.igrejaData = $rootScope.USER.getContextByKeyAndId(Contexts.EVANGELISTAS, $scope.igreja);
    ...
    $scope.localPerms = {
        add: 'AgendaEvangelistaSave',
        edit: 'AgendaEvangelistaSave',
        changeStat: 'AgendaEvangelistaBlock',
        remove: 'AgendaEvangelistaRemove'
    };
    ...
    $scope.evangelistaId = $rootScope.USER.getOficialId('evangelista', $scope.igreja);
    Data.getAll().then(function(r) {
        $scope.listStatus = r.status;
        $scope.listUFs = r.ufs;
        $scope.listCidades = r.cidades;
        TagAgenda.getAll('', '', '', '', '', '', 'geral,evangelista').then(function(r) {
            if(r.total > 0) {
    ...
    /* search */
    $scope.storage_cache_name = 'search_agendadoevangelista';
    $scope.searchBy = '';
    $scope.list = [];
    $scope.createSearchObject = function(only_non_existent) {
        ...
        $localstorage.setObject($scope.storage_cache_name, {
            ...
            termino: '',
            stat: '',
            responsavel: $scope.evangelistaId,
            recorrente: false,
            domingo: false,
            segunda: false,
            terca: false,
    ...
    $scope.clear = function() {
        ...
        $scope.termino = '';
        $scope.stat = '';
        $scope.responsavel = $scope.evangelistaId;
        ...
        if($scope.list.length > 0) {
            $scope.calendarObj = $('#calendar').fullCalendar({
                ...
                eventRender: function(event, element) {
                    ...
                    var opts = '';
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaEvangelistaBlock')) {
                        if(event.stat == 'ATV')
                        {
                    ...
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaEvangelistaSave')) {
                        opts += '&nbsp;<button type="button" class="btn btn-warning btn-xs calendar-edit" data-popover-id="'+popover_id+'"> '+
                                    '    <i class="fas fa-edit"></i>'+
                                '</button>'
                            ;
                    }
                    if($scope.USER.doIHavePermission($scope.user, 'AgendaEvangelistaRemove')) {
    ...
    $scope.toEdit = function(id) {
        $state.go('SmartChurchPanel.evangelista.agenda.editar', {id: id, title: 'Editando', back: 'SmartChurchPanel.evangelista.agenda.buscar'});
    };
