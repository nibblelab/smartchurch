angular.module('SmartChurchPanel').controller('SalaEBDCtrl', function ($scope, $state, $stateParams, $localstorage, $notifications, 
    ...
    $scope.back = ($stateParams.back == '') ? 'dashboard.salasebd.buscar()' : $stateParams.back + '()';
    $scope.id = $stateParams.id;
    $scope.objForm = 'Sala da Escola Dominical';
    $scope.objList = 'Salas da Escola Dominical';
    ...
    $scope.localPerms = {
        add: 'SalaEBDSave',
        edit: 'SalaEBDSave',
        changeStat: 'SalaEBDBlock',
        remove: 'SalaEBDRemove',
        professores: 'SalaEBDProfessores',
        alunos: 'SalaEBDAlunos',
        frequencias: 'FrequenciaEBD',
        sumario: 'SumarioEBD'
    };
    ...
    /* search */
    $scope.storage_cache_name = 'search_salasebd';
    $scope.searchBy = '';
    $scope.page = 1;
    $scope.pageSize = '10';
    $scope.hasPrev = false;
    ...
    $scope.doSearch = function(is_new) {
        ...
        $scope.list = [];
        SalaEBD.getAll($scope.page, $scope.pageSize, $scope.searchBy, $scope.orderBy, '', $scope.stat, 
    ...
    $scope.dataFrm = {
        ...
        prepare: function() {
            ...
            if($scope.dataFrm.data.inicio.value != '') {
                $scope.dataFrm.toSend.inicio = moment($scope.dataFrm.data.inicio.value, 'DD/MM/YYYY').format('YYYY-MM-DD');
            }
            
            if($scope.dataFrm.data.fim.value != '') {
                $scope.dataFrm.toSend.fim = moment($scope.dataFrm.data.fim.value, 'DD/MM/YYYY').format('YYYY-MM-DD');
            }
        }
    };
    ...
    if ($state.current.name.includes('editar') && $scope.id) {
        // edição - via id 
        $scope.$on('preLoad', function() {
            SalaEBD.getMe($scope.id).then(function(r) {
                $scope.data = r;
                $scope.load();
            }, function(e) { 
    ...
    $scope.load = function() {
        ...
        if($scope.data.inicio != null) {
            var t_inicio = moment($scope.data.inicio);
            $scope.dataFrm.data.inicio.value = t_inicio.format('DD/MM/YYYY');
        }
        else {
            $scope.dataFrm.data.inicio.value = '';
        }
        
        if($scope.data.fim != null) {
            var t_fim = moment($scope.data.fim);
            $scope.dataFrm.data.fim.value = t_fim.format('DD/MM/YYYY');
        }
        else {
    ...
    $scope.doSave = function() {
        ...
        if ($scope.dataFrm.isValid) {
            ...
            if($scope.isCreate)
            {
                promise = SalaEBD.create($scope.dataFrm.toSend);
            }
            else
            {
                promise = SalaEBD.edit($scope.id, $scope.dataFrm.toSend);
            }
    ...
    $scope.changeStat = function(d) {
        $dialogs.beforeChange().then(function() {
            SalaEBD.changeStat(d.id, d).then(function(r) {
    ...
    $scope.remove = function(d) {
        ...
        $dialogs.beforeRemove().then(function() {
            SalaEBD.remove(d.id).then(function() {
                $timeout(function() {
